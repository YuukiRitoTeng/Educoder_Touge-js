...
(function () {
  ...
  const STORE_KEY_AUTO_REFRESH_DONE_AT = "TG_TASK_ASSISTANT_AUTO_REFRESH_DONE_AT";
  ...
  async function loadAutoRefreshDoneAt() {
    return Number(await getValue(STORE_KEY_AUTO_REFRESH_DONE_AT, 0)) || 0;
  }

  async function markAutoRefreshDoneAt(timestamp = Date.now()) {
    await setValue(STORE_KEY_AUTO_REFRESH_DONE_AT, Number(timestamp) || Date.now());
  }

  async function clearAutoRefreshDoneAt() {
    await deleteValue(STORE_KEY_AUTO_REFRESH_DONE_AT);
  }
  ...
  async function maybeTriggerAutoRefreshOnce(source = "frontend-panel-open") {
    ...
    const autoRefreshDoneAt = await loadAutoRefreshDoneAt();
    if (autoRefreshDoneAt) return false;

    panelAutoRefreshRequested = true;
    await markAutoRefreshDoneAt(Date.now());
  ...
  async function measureCurrentTgLatency() {
    ...
    try {
      ...
      const site = getCurrentPageSite();
      if (site.siteKey !== "tg-zcst") throw new Error("当前页面不是 TG 页面");
      const logs = Array.isArray(state.result?.debug?.requestLog) ? state.result.debug.requestLog : [];
      const entry = logs.slice().reverse().find(item => item?.url && item?.ok !== false && (item.siteKey === "tg" || item.siteKey === "tg-zcst"));
      if (!entry?.url) throw new Error("没有可复用的已验证 TG 只读 endpoint");
      const endpoint = new URL(entry.url, location.href);
      if (endpoint.origin !== location.origin) throw new Error("已验证 endpoint 与当前 TG Origin 不一致");
      const samples = [];
      for (let i = 0; i < 3; i += 1) {
  ...
  function maybeRunInitialLatencyTest() {
    if (initialLatencyTestRequested || ![TG_INTRANET_HOST, "tg.zcst.edu.cn"].includes(location.hostname)) return;
  ...
  async function requestBackendRefresh(source, type = "manual", options = {}) {
    ...
    if (location.hostname === "www.educoder.net") {
      try {
        ...
        await markAutoRefreshDoneAt(activeRefreshStartedAt);
      } catch (err) {
    ...
    await markAutoRefreshDoneAt(requestedAt);
    ...
    pollTimerId = setInterval(async () => {
      ...
      if (presentation.hasCurrentTerminalStatus) {
        if (presentation.statusName === "done") {
          await markAutoRefreshDoneAt(refreshStatus?.timestamp || Date.now());
        }
        activeRefreshRequestId = "";
        activeRefreshStartedAt = 0;
        render();
